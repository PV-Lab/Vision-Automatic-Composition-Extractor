...
def get_compositions(segmentation, motor_speeds, gcode, rasterSpeed, maxComp=100, minComp=0, bufferTimeStart=1,
                     bufferTimeEnd=1, rasterScaleX=1, rasterScaleY=1, rasterOffsetX=0, rasterOffsetY=0):
    ####################
    # Motor Speed Data #
    ####################
    ...
    plt.plot(motor_speeds_full[:, 0], motor_speeds_full[:, 1], label='Motor A')
    plt.plot(motor_speeds_full[:, 0], motor_speeds_full[:, 2], label='Motor B')
    plt.xlabel('Time [ms]')
    ...
    for d in drop_idx:
        ...
        FA = (dBdt / (dAdt + dBdt)).round(2)
        MA = (dAdt / (dAdt + dBdt)).round(2)
        composition = f'FA{FA}MA{MA}'
        t = ax.text((xstart + xend) / 2, (ystart + yend) / 2,
                    'FA%g' % ((FA * 100).round(0)) + '\nMA%g' % ((MA * 100).round(0)), ha='center', fontsize=8,
